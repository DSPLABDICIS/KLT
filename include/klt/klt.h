...
#define KLT_LANDSCAPE        -6

//#define LIM 1.5
#define LIM 0.1
//#define MINI 1 //conflict with OpenCV
#include "klt_util.h"/* for affine mapping */
#include "base.h" /* for uchar */
//#include "convolve.h"
/*******************
 * Structures
 */
...
typedef struct  {
  KLT_locType x, vx, errorx, xem, xpro, zem;
  KLT_locType y, vy, errory, yem, ypro, zpro;
  KLT_locType vz;
  KLT_locType _3Dx;             //3d variable
  KLT_locType _3Dy;             //3d variable
  KLT_locType _3Dz; 			//3d variable
  int _3Dlost;
  int val, errorval, auxval, trail;
  /* for affine mapping */
...
void KLTWriteFeatureListToPPM(
  ...
  int nrows,
  char *filename);
void KLTWriteFeatureListToImg(
	KLT_FeatureList fl,
	KLT_PixelType *greyimg,
	int ncols,
	int nrows,
	KLT_PixelType *RGBimg);
...
void KLTWriteFeatureTableReduced(
  ...
  const char *fmt,
  int _3D);
void writeFeatureTableForClusters(
  KLT_FeatureTable ft,
  char *fname,
  char *fmt);
void writeFeatureTableForSLAM(
  KLT_FeatureTable ft,
  char *fname,
  char *fmt);
