...
static const KLT_BOOL sequentialMode = TRUE; //FALSE
...
static const float affine_max_residue = 12.0;
...
static const KLT_BOOL writeInternalImages = FALSE;   // FALSE
...
void KLTFreeElementsOfFeatureList(
  KLT_FeatureList fl)
{
  /* for affine mapping */
  int indx;
...
void KLTFreeFeatureTable(
  KLT_FeatureTable ft)
{
  if(ft->feature != NULL) free(ft->feature[0][0]);  /* this plugs a memory leak found by Stefan Wachter */
  free(ft->feature);
  free(ft);
}
  ...
  KLT_FeatureList fl)
{
  ...
  for (i = 0 ; i < fl->nFeatures ; i++){
    if ( (fl->feature[i]->val == 0)  &&  ((fabs(fl->feature[i]->vx) > LIM ) || ( fabs(fl->feature[i]->vy) > LIM)) )
      count++;
  }
  return count;
}
...
int *KLTFindIndexOfLostFeatures(
				KLT_FeatureList fl, int count)
{
