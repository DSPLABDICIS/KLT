...
static int _am_trackFeatureAffine(
				  ...
				  float *x2, /* starting location of search in second image */
				  float *y2,
				  _KLT_FloatImage img1,  // current image patch
				  _KLT_FloatImage gradx1,
				  _KLT_FloatImage grady1,
				  _KLT_FloatImage img2,  // initial image patch
				  _KLT_FloatImage gradx2,
				  _KLT_FloatImage grady2,
				  int width,           /* size of window */
				  int height,
				  ...
				  float *Axy, float *Ayy)        /* used affine mapping */
{
  ...
  do  {
    if(!affine_map) {
      ...
      sprintf(fname, "kltimg_trans_diff_win%03d.%03d.pgm", glob_index, counter);
      printf("%s\n", fname);
      ...
      *y2 += dy;
      ...
      aff_diff_win->data = imgdiff;
      sprintf(fname, "kltimg_aff_diff_win%03d.%03d_1.pgm", glob_index, counter);
      printf("%s\n", fname);
      _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);

      _am_computeAffineMappedImage(img2, *x2, *y2,  *Axx, *Ayx , *Axy, *Ayy, width, height, imgdiff);
      aff_diff_win->data = imgdiff;
      sprintf(fname, "kltimg_aff_diff_win%03d.%03d_2.pgm", glob_index, counter);
      printf("%s\n", fname);
      _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
#endif
...
#ifdef DEBUG_AFFINE_MAPPING
      aff_diff_win->data = imgdiff;
      sprintf(fname, "kltimg_aff_diff_win%03d.%03d_3.pgm", glob_index,counter);
      printf("%s\n", fname);
      _KLTWriteAbsFloatImageToPGM(aff_diff_win, fname,256.0);
					  ...
					  KLT_FeatureList featurelist)
{
	...
	for (indx = 0 ; indx < featurelist->nFeatures ; indx++)  {
		...
		if (featurelist->feature[indx]->val >= 0)  {
			...
			// to save position in image t-1 (mainly during object tracking)
			...
			//	featurelist->feature[indx]->y   = -1.0;
				featurelist->feature[indx]->vx   = -1.0;
				featurelist->feature[indx]->vy   = -1.0;
				featurelist->feature[indx]->vz   = -1.0;
				...
				if (tc->affineConsistencyCheck >= 0 && val == KLT_TRACKED)  { /*for affine mapping*/
		       	...
		       	/* save image and gradient for each feature at finest resolution after first successful track */
						...
						val = _am_trackFeatureAffine(featurelist->feature[indx]->aff_x, featurelist->feature[indx]->aff_y,
							...
							tc->min_determinant,
							tc->min_displacement,
							tc->affine_min_displacement, //0.02
						        tc->affine_max_residue,  //10
							tc->lighting_insensitive,
