		...
		int _3D)				/* Print 3D information*/
{
	...
	if(_3D)
		sprintf(format, "(%s,%s | %s, %s | %s, %s, %s |  %s, %s, %s | %%d)=%%%dd ", fmt, fmt,fmt, fmt,fmt, fmt,fmt, fmt, fmt, fmt, val_width);
	else
		sprintf(format, "(%s,%s | %s,%s | %s, %s)=%%%dd ", fmt, fmt,fmt, fmt, fmt, fmt, val_width);

	return fp;
}

static FILE* _printSetupTxtReduced(
		char *fname, 			/* Input: filename, or NULL for stderr */
		const char *fmt,		/* Input: format (e.g., %5.1f or %3d) */
		char *format,			/* Output: format (e.g., (%5.1f,%5.1f)=%3d) */
		char *type,			/* Output: either 'f' or 'd', based on input format */
		int _3D)				/* Print 3D information*/
{
	...
	/* Construct feature format */
	if(_3D)
		sprintf(format, "(%s, %s, %s |  %s, %s, %s | %%3d, %%d)=%%%dd ", fmt, fmt,fmt, fmt,fmt, fmt, val_width);
	else
		sprintf(format, "(%s,%s | %s, %s)=%%%dd ", fmt, fmt, fmt, fmt, val_width);
		...
		int _3D)
{
	...
	if (type == 'f')
		if(_3D)
			fprintf(fp, format, (float) feat->x, (float) feat->y, (float) feat->xem, (float) feat->yem,
					(float) feat->_3Dx, (float) feat->_3Dy, (float) feat->_3Dz, (float) feat->vx, (float) feat->vy, (float) feat->vz, feat->_3Dlost, feat->val);
		else
			fprintf(fp, format, (float) feat->x, (float) feat->y, (float) feat->xem, (float) feat->yem, (float) feat->vx, (float) feat->vy, feat->val);
	else if (type == 'd')  {
		...
		if (y >= 0.0) y += 0.5;
		if(_3D)
			fprintf(fp, format, (int) x, (int) y, (int) feat->xem, (int) feat->yem,
					(int) feat->_3Dx, (int) feat->_3Dy, (int) feat->_3Dz, (int) feat->vx, (int) feat->vy, (int) feat->vz, feat->val);
		else
		fprintf(fp, format,
				(int) x, (int) y,(int) feat->xem, (int) feat->yem, (int)feat->vx, (int)feat->vy, feat->val);
	}
}

static void _printFeatureTxtReduced(
		FILE *fp,
		KLT_Feature feat,
		char *format,
		char type,
		int _3D)
{
	...
	if (type == 'f')
		if(_3D)
			fprintf(fp, format,(float) feat->_3Dx, (float) feat->_3Dy, (float) feat->_3Dz, (float) feat->vx, (float) feat->vy, (float) feat->vz, feat->trail, feat->_3Dlost, feat->val);
		else
			fprintf(fp, format, (float) feat->x, (float) feat->y, (float) feat->xem, (float) feat->yem, (float) feat->vx, (float) feat->vy, feat->val);
	else if (type == 'd')  {
		/* Round x & y to nearest integer, unless negative */
		if(_3D)
			fprintf(fp, format, (int) feat->_3Dx, (int) feat->_3Dy, (int) feat->_3Dz, (int) feat->vx, (int) feat->vy, (int) feat->vz, feat->val);
		else{
			float x = feat->x;
			float y = feat->y;
			if (x >= 0.0) x += 0.5;
		...
		int _3D)
{
	...
	if (fmt != NULL) {  /* text file or stderr */
		fp = _printSetupTxt(fname, fmt, format, &type, _3D);
		_printHeader(fp, format, FEATURE_LIST, 0, fl->nFeatures);

		for (i = 0 ; i < fl->nFeatures ; i++)  {
			fprintf(fp, "%7d | ", i);
			_printFeatureTxt(fp, fl->feature[i], format, type, _3D);
			fprintf(fp, "\n");
		}
		_printShutdown(fp);
	} else {  /* binary file */
		...
		int _3D)
{
	...
	if (fmt != NULL) {  /* text file or stderr */
		...
		for (i = 0 ; i < fh->nFrames ; i++)  {
			fprintf(fp, "%5d | ", i);
			_printFeatureTxt(fp, fh->feature[i], format, type, _3D);
			fprintf(fp, "\n");
		}
		_printShutdown(fp);
	} else {  /* binary file */
	...
	if (fmt != NULL) {  /* text file or stderr */
		...
		for (j = 0 ; j < ft->nFeatures ; j++)  {
			...
			for (i = 0 ; i < ft->nFrames ; i++)
				_printFeatureTxt(fp, ft->feature[j][i], format, type, _3D);
			fprintf(fp, "\n");
		}
		_printShutdown(fp);
	} else {  /* binary file */
...
void KLTWriteFeatureTableReduced(
		KLT_FeatureTable ft,
		char *fname,
		const char *fmt,
		int _3D)
{
	...
	if (fmt != NULL) {  /* text file or stderr */
		fp = _printSetupTxtReduced(fname, fmt, format, &type, _3D);
		_printHeader(fp, format, FEATURE_TABLE, ft->nFrames, ft->nFeatures);
		if(_3D)
			fprintf(fp,"(3Dx, 3Dy, 3Dz | vx, vy, vz | trail, 3Dlostflag) = val\n");
		else
			fprintf(fp,"(x, y | vx, vy ) = val\n");

		for (j = 0 ; j < ft->nFeatures ; j++)  {
			fprintf(fp, "%7d | ", j);
			for (i = 0 ; i < ft->nFrames ; i++)
				_printFeatureTxtReduced(fp, ft->feature[j][i], format, type, _3D);
			fprintf(fp, "\n");
		}
		_printShutdown(fp);
	} else {  /* binary file */
...
void writeFeatureTableForClusters( KLT_FeatureTable ft,
		char *fname,
		char *fmt)
{
	FILE *fp;
	...
	if (KLT_verbose >= 1 && fname != NULL)  {
		fprintf(stderr,
				"(KLT) Writing feature table for clusters to %s file: '%s'\n",
				(fmt == NULL ? "binary" : "text"), fname);
	}
	if (fname == NULL)  fp = stderr;
	else  fp = fopen(fname, "wb");
	...
	for (j = 0 ; j < ft->nFeatures ; j++)  {
		...
		for (i = 0 ; i < ft->nFrames ; i++){
			if (i==0){
				fprintf(fp, format, (float) ft->feature[j][i]->x, (float) ft->feature[j][i]->y, (float) ft->feature[j][i]->vx, (float) ft->feature[j][i]->vy,(int) ft->feature[j][i]->val );
			}
				...
				if (type == 'f')
					...
					/* Round x & y to nearest integer, unless negative */
					float x = ft->feature[j][i]->x;
					float y = ft->feature[j][i]->y;
					if (x >= 0.0) x += 0.5;
					if (y >= 0.0) y += 0.5;
					fprintf(fp, format,
							(int) x, (int) y, (float) ft->feature[j][i]->vx, (float) ft->feature[j][i]->vy,(int) ft->feature[j][i]->val);
				} //end elsif
			} //end else
		} //end for frames
		fprintf(fp, "\n");
...
void writeFeatureTableForSLAM( KLT_FeatureTable ft5,
		char *fname,
		char *fmt)
{
	FILE *fp;
	...
	char type;
	int i, j;
	//int trialS=3, val_width=5;

	if (KLT_verbose >= 1 && fname != NULL)  {
		fprintf(stderr,
				"(KLT) Writing feature table for SLAM to %s file: '%s'\n",
				(fmt == NULL ? "binary" : "text"), fname);
	}
	if (fname == NULL)  fp = stderr;
	else  fp = fopen(fname, "wb");
	...
	/* Construct feature format */
	//  sprintf(format, "(%s\t%s\t%%%dd\t%%%dd)", fmt,fmt,val_width, trialS );
	//  sprintf(format, "%s\t%s\t %s\t%s\t || ", fmt,fmt, fmt, fmt);
	sprintf(format, "(%s\t%s) ", fmt,fmt); // para SLAM cooperacion
	int  vals, trials;

	for (j = 0 ; j < ft5->nFeatures ; j++)  {
		// Test static
		trials=0;
		for (i = 0 ; i < ft5->nFrames ; i++){
			trials+=(int) ft5->feature[j][i]->trail;
			(i!=0) ? (vals += (int)ft5->feature[j][i]->val) : (vals=0);
		}
		//    if(trials ==0 && vals ==0){  // para SLAM
		if( ((trials ==0)||(trials >3)) && vals ==0){    // para evaluacion de ego motion
			//  fprintf(fp, "%7d | ", j);
			for (i = 0 ; i < ft5->nFrames ; i++){
				if (i==0){
					//	  fprintf(fp, format, (float) ft5->feature[j][i]->x, (float) ft5->feature[j][i]->y,(int) ft5->feature[j][i]->val, (int) ft5->feature[j][i]->trail );
					//	  fprintf(fp, format, (float) ft5->feature[j][i]->x, (float) ft5->feature[j][i]->y, ft5->feature[j][i]->xem, (float) ft5->feature[j][i]->yem );
					fprintf(fp, format, (float) ft5->feature[j][i]->x, (float) ft5->feature[j][i]->y); // para SLAM cooperacion
				}
					...
					if (type == 'f')
						...
						/* Round x & y to nearest integer, unless negative */
						float x = ft5->feature[j][i]->x;
						float y = ft5->feature[j][i]->y;
						if (x >= 0.0) x += 0.5;
						if (y >= 0.0) y += 0.5;
						float xem = ft5->feature[j][i]->xem;
						float yem = ft5->feature[j][i]->yem;
						if (xem >= 0.0) xem += 0.5;
						if (yem >= 0.0) yem += 0.5;
						//   fprintf(fp, format,
						//    (int) x, (int) y, (int) ft5->feature[j][i]->val, (int) ft5->feature[j][i]->trail);
						//  fprintf(fp, format, (int) x, (int) y, (int)xem, (int)yem );
						fprintf(fp, format, (int) x, (int) y); //para SLAM cooperacion
					} //end elsif
				} //end else
			} //end for frames
			fprintf(fp, "\n");
